...
Term_Logical =
	head:Term_Logical_Inverse tail:( _ ("∧" / "∨") _ Term_Logical_Inverse)* {
...
Term_Logical_Inverse =
	"¬" _ "(" _ body:Relationship _ ")" {
		return ["not", body]
	}
	/ Relationship
...
Term_Addition_Subtraction =
	head:Term_Multiplication_Division tail:( _ ("+" / "-") _ Term_Multiplication_Division)* {
		let result = tail.reduce((head, tail) =>{
				let [/*whitespace*/, op, /*whitespace*/, term] = tail;
				if(op==="+"){
