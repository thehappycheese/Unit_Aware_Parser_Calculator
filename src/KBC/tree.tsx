		...
		if (inline) {
			...
			return (
				<div className="Tree-Non-Indent">
					...
					{intersperse(tail.map(item => print_JSX(item, inline))," ")}{" "}
					] 
				</div>
			);
		} else {
			// indented recursion
			let go_inline = inline || max_depth(tree)<4;
			return (
				<div>
					<div>[<span className="Tree-Function-Name">{head}</span></div>
						<div className="Tree-Indent">{intersperse(tail.map(item => print_JSX(item, go_inline)),go_inline?<br/>:"")}</div>
					<div>]</div>
				</div>
			);
		}
